...
function AdminDashboard() {
  ...
  const fetchUser = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8080/admin/getDetails/${userId}`,
        { withCredentials: true },
      );
      ...
      toast.error(
        error.response?.data || "Failed to fetch user or User not found",
      );
  ...
  const handleMBusinessForm = async (e) => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:8080/admin/analytics/month",
        {
  ...
  const handleYearlyReportsForm = async (e) => {
    try {
      e.preventDefault();
      const response = await axios.get(
        "http://localhost:8080/admin/analytics/year",
        {
          params: { year: parseInt(e.target.year.value) },
          withCredentials: true,
        },
      );

      toast.success("Yearly reports data fetched successfully");
      setReportYear(response.data.yearly_data);
      setResponseYear(true);
    } catch (error) {
      console.error("Error fetching yearly reports data:", error);
      toast.error("Failed to fetch yearly reports data");
    }
  };
  ...
  return (
    ...
    <Navbar
      ...
      <div className="admin-dashboard-container grid grid-cols-2 gap-4 m-10 flex flex-wrap justify-center">
        ...
        {/* 6) Reports */}
        <DashboardCard
          buttonText="Yearly Reports"
          title="Yearly Reports Overview"
          subtitle="Activity : Analytics"
          onClick={() => setActivePopup("YEARLY_REPORTS")}
        />
      </div>
      ...
      {activePopup === "DELETE_PRODUCT" && (
        <PopupModal
          ...
          onClose={() => {
            (closePopup(), setProductId(""), setProduct(null));
          }}
        >
          <form onSubmit={handleFormDeletion} className="space-y-3">
            <div className="flex gap-3 pt-2 justify-center">
              ...
              <button
                type="button"
                onClick={(e) => fetchProduct(e)}
                className="bg-green-400 hover:bg-green-300 rounded px-4 py-2"
              >
                Fetch
              </button>
            </div>
            ...
            {product && (
              <div className="mt-4 p-4 border-2 border-green-400 rounded gap-2">
                <h3 className="font-bold text-lg mb-2">Product Details</h3>
                <p className="bg-green-100 rounded p-2 mb-2 font-bold">
            ...
            <div className="flex justify-center">
              <button
                type="submit"
                className="bg-red-600 text-white px-4 py-2 mt-4  rounded hover:bg-red-700 transition-colors duration-300 ease-in-out "
              >
                Delete Product
              </button>
            </div>
        ...
        >
          <div className="flex  items-center justify-center">
            ...
            <input
              ...
              name="userId"
              id="userId"
              className="border p-2 ml-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={userId}
              onChange={(e) => setUserId(e.target.value)}
            />
            <button
              className="bg-green-500 hover:bg-green-400 p-2 ml-4 rounded w-20 text-white"
          ...
          <form className="mt-4 space-y-4 relative" onSubmit={handleUserForm}>
            ...
            <label
              htmlFor="role"
              className="absolute top-30 left-2 bg-white px-1 font-semibold"
            >
              Role{" "}
            </label>
            <input
            ...
            <label
              ...
              className="absolute top-46 left-2 bg-white px-1 font-semibold"
            >
              Auth Provider{" "}
            </label>
            <input
        ...
        >
          {!showMonthlyChart ? (
            ...
            <div className="mt-4 flex flex-col items-center">
              ...
              <p className="font-semibold mb-4">Total Amount: ₹{totalAmount}</p>
              ...
              <PieChart width={400} height={300}>
                ...
                <div className="flex justify-center">
                  <p className="font-semibold text-gray-600">No data available for the selected month and year.</p>
                </div>
              )}
            </div>
          )}
      ...
      {activePopup === "YEARLY_REPORTS" && (
        <PopupModal
          title="Yearly Reports Overview"
          onClose={() => {
            closePopup(),
            setReportYear([]),
            setResponseYear(false);}}
        >
          {!responseYear ? (
            <form
              onSubmit={handleYearlyReportsForm}
              className="space-y-3 flex mt-4 justify-center"
            >
              <label htmlFor="year" className="m-1">
                Year
              </label>
              <input
                type="number"
              ...
              <button
                type="submit"
                className=" ml-2 bg-green-600 text-white px-4 py-2 h-9 rounded hover:bg-green-700 transition-colors duration-300 ease-in-out"
              >
                Submit
              </button>
            </form>
            ...
            { reportYear.length > 0 ? (
              <div style={{ width: "50%", height: 350 }}>
                <ResponsiveContainer>
                  <BarChart
                    data={reportYear.map((item, index) => ({
                      month: item.month,
            ...
            <p className="font-semibold text-gray-600">No data available for the selected year.</p>
            </div>
            ) }
            </div>
          )}
